...
function renderPaymentMethods(){
  ...
  enabled = enabled.filter(m=> !(m.id==='cod' && m.khartoumOnly && city && !isKhartoum));
  ...
  document.getElementById('noPayMethodMsg').textContent = codBlockedByCity && enabled.length<=1
    ? 'الدفع عند الاستلام متاح داخل الخرطوم بس — لباقي الولايات استخدم التحويل البنكي'
    : 'لا توجد طريقة دفع مفعّلة حاليًا — تواصل معنا';
  ...
  if(codBlockedByCity && enabled.length && enabled.every(m=>m.id!=='cod')){
    box.insertAdjacentHTML('afterbegin', `<p class="pm-note" style="color:var(--hazard-400);">📍 الدفع عند الاستلام متاح داخل الخرطوم بس — اخترنا لك التحويل البنكي بدله</p>`);
  }
