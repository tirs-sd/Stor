...
db.collection('settings').doc('main').onSnapshot(doc=>{
  ...
  document.getElementById('phoneDisplay').textContent = settings.phone2 || settings.whatsappNumber || 'اتصل بنا';
  document.getElementById('topBarPhone').href = 'tel:' + (settings.phone2 || settings.whatsappNumber || '');
...
const quickIcons = document.getElementById('catIconStrip');
...
function buildProductCard(p){
    ...
    card.innerHTML = `
      <div class="tag-img">${hasDiscount?`<span class="discount-badge">خصم ${discountPct}%</span>`:''}${p.imageUrl?`<img src="${escAttr(p.imageUrl)}">`:icon(p.icon)}</div>
      ...
      <div class="tag-price-row">
        <div>
          ${hasDiscount?`<div class="price-strike">${compareAt.toLocaleString('en-US')} ج.س</div>`:''}
...
function renderFeatured(){
  ...
  grid.innerHTML='';
  const inStock = products.filter(p=> (Number(p.qty)||0) > 0);
  const shuffled = inStock.slice().sort(()=> Math.random()-0.5).slice(0,4);
  shuffled.forEach(p=> grid.appendChild(buildProductCard(p)));
  grid.querySelectorAll('.add-btn:not([disabled])').forEach(btn=> btn.addEventListener('click', ()=> addToCart(btn.dataset.id)));
}
document.getElementById('quoteCtaBtn').addEventListener('click', ()=> goToPage('support'));
...
document.getElementById('vehStripBtn').addEventListener('click', ()=>{
  goToPage('home');
...
function runHeaderSearch(){
  ...
  if(!val) return;
  goToPage('home');
  document.getElementById('storeSearch').value = val;
  setFilter('all');
  renderProducts();
  setTimeout(()=> document.getElementById('bestsellers').scrollIntoView({behavior:'smooth'}), 60);
}
document.getElementById('headerSearch').addEventListener('keydown', (e)=>{ if(e.key==='Enter') runHeaderSearch(); });
