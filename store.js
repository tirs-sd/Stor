...
document.querySelectorAll('input[name="payMethod"]').forEach(r=>{
  r.addEventListener('change', ()=>{
    document.getElementById('bankakBox').style.display = r.value==='تحويل بنكك' && r.checked ? 'block' : 'none';
  });
});
...
document.getElementById('checkoutBtn').addEventListener('click', async ()=>{
  ...
  if(payMethod==='تحويل بنكك' && !receiptFile){ showToast('ارفع صورة إيصال الحوالة الأول'); return; }
  ...
  try{
    ...
    const order = {
      ...
      paymentMethod: payMethod, paymentStatus: payMethod==='تحويل بنكك' ? 'قيد المراجعة' : 'عند الاستلام', receiptUrl,
    ...
    document.getElementById('successPayNote').textContent = payMethod==='تحويل بنكك' ? 'راجعنا إيصالك وهنأكد الطلب قريبًا' : 'ادفع نقدًا وقت استلام القطعة';
...
auth.onAuthStateChanged(async (user)=>{
  if(user && !user.isAnonymous){
    ...
    customerProfile = doc.exists ? doc.data() : {name:user.email, phone:'', city:''};
    showProfileView();
...
document.getElementById('supSubmit').addEventListener('click', async ()=>{
  ...
  if(!title || !phone){ showToast('اكتب الموضوع ورقم الجوال الأول'); return; }
  const ref = await db.collection('tickets').add({
    scope:'customer', category, title: title + (msg?(' — '+msg):''), customer: customerProfile?customerProfile.name:'', phone,
  ...
  logEvent('تذكرة جديدة', 'tickets', ref.id, title);
  showToast('تم إرسال تذكرتك، هنرد عليك قريبًا');
  document.getElementById('supTitle').value=''; document.getElementById('supMsg').value=''; document.getElementById('supPhone').value='';
});
