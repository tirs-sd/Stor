...
const PERMS = [
  ...
  {key:'customers', label:'عملاء', views:['customers']},
...
const NAV_ITEMS = [
  ...
  {view:'customers', perm:'customers', label:'العملاء', icon:'<circle cx="12" cy="8" r="4"/><path d="M4 21c0-4.4 3.6-8 8-8s8 3.6 8 8"/>'},
...
let orders=[], tickets=[], employees=[], products=[], coupons=[], invoices=[], events=[], notifications=[], customers=[], settings={storeName:'تِرس', exchangeRate:0};
...
function startListeners(){
  ...
  db.collection('customers').onSnapshot(snap=>{
    customers = snap.docs.map(d=>({id:d.id, ...d.data()})); renderCustomers();
  });
...
function productRow(p){
  ...
  return `<tr data-id="${p.id}">
    ...
    <td><div class="img-upload-cell" data-dropzone="${p.id}">
    ...
    <td><input class="editable-text mono" data-field="compareAtPrice" type="number" min="0" step="500" value="${p.compareAtPrice||''}" placeholder="بدون خصم" title="اتركه فاضي لو مفيش خصم — لو حطيت رقم أكبر من السعر، هتظهر شارة خصم للعميل"></td>
...
document.getElementById('productTable').addEventListener('change', (e)=>{
  ...
  if(field){
    const id = e.target.closest('tr').dataset.id;
    if(field==='price' || field==='priceUSD' || field==='compareAtPrice'){ db.collection('products').doc(id).update({[field]: parseFloat(e.target.value)||0}); }
    else if(field==='qty'){
...
function uploadProductImage(id, file){
  if(!file) return;
  if(!storage){ showToast('⚠️ رفع الصور غير مفعّل — فعّل Storage من كونسول Firebase أولًا (شرحها بالدليل)'); return; }
  if(!file.type.startsWith('image/')){ showToast('اختر ملف صورة صحيح'); return; }
  ...
  }).catch(err=> showToast('تعذر رفع الصورة: ' + (err.code || err.message || 'خطأ غير معروف')));
}
document.getElementById('productTable').addEventListener('dragover', (e)=>{
  const zone = e.target.closest('[data-dropzone]'); if(!zone) return;
  e.preventDefault(); zone.classList.add('drag-over');
});
document.getElementById('productTable').addEventListener('dragleave', (e)=>{
  const zone = e.target.closest('[data-dropzone]'); if(!zone) return;
  zone.classList.remove('drag-over');
});
document.getElementById('productTable').addEventListener('drop', (e)=>{
  const zone = e.target.closest('[data-dropzone]'); if(!zone) return;
  e.preventDefault(); zone.classList.remove('drag-over');
  const file = e.dataTransfer.files[0];
  uploadProductImage(zone.dataset.dropzone, file);
});
  if(e.target.dataset.action==='upload-img'){
    const id = e.target.closest('tr').dataset.id;
    uploadProductImage(id, e.target.files[0]);
  }
});
...
function ticketRow(t){
  ...
  const repliesHtml = (t.comments||[]).map(c=>`<div class="comment-item ${c.visibleToCustomer?'customer-visible':'staff-msg'}">
      <span class="time">${arDate(c.date)}</span>
      <b>${escAttr(c.author)}</b> ${c.visibleToCustomer?'<span class="vis">✓ مرئي للعميل</span>':'<span class="internal">داخلي فقط</span>'}
      <p>${escAttr(c.text)}</p>
...
function customerOrders(c){ return orders.filter(o=> (o.customerUid && o.customerUid===c.id) || (!o.customerUid && c.phone && o.phone===c.phone)); }
function customerInvoices(c){ return invoices.filter(inv=> (inv.customerUid && inv.customerUid===c.id) || (!inv.customerUid && c.phone && inv.phone===c.phone)); }
...
function renderCustomers(){
  ...
  document.getElementById('customersTable').innerHTML = list.map(c=>{
    ...
    const cInvoices = customerInvoices(c);
    ...
    const rows = `<tr data-id="${c.id}">
      ...
      <td class="mono" style="color:${outstanding>0?'var(--stock-red)':'var(--stock-green)'};">${outstanding.toLocaleString('ar-SD')} ج.س</td>
      <td><button class="icon-btn-sm" data-action="toggle-cust-details" title="التفاصيل"><svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="M6 9l6 6 6-6"/></svg></button></td>
    </tr>`;
...
const DEFAULT_PAY_METHODS = [{id:'cod', label:'نقدي عند الاستلام', enabled:true, khartoumOnly:true}, {id:'bankak', label:'تحويل عبر بنكك', enabled:true}];
...
function renderPayMethodsTable(){
  document.getElementById('payMethodsTable').innerHTML = payMethods().map(m=>`
    <tr data-pmid="${m.id}">
      ...
      <td>${m.id==='cod' ? `<label style="display:flex;align-items:center;gap:8px;"><input type="checkbox" data-pmfield="khartoumOnly" ${m.khartoumOnly?'checked':''} style="width:16px;height:16px;"> يظهر بالخرطوم بس</label>` : '<span style="color:var(--steel-400);font-size:.72rem;">—</span>'}</td>
    </tr>`).join('');
}
document.getElementById('savePayMethodsBtn').addEventListener('click', ()=>{
  const updated = [...document.querySelectorAll('#payMethodsTable tr')].map(tr=>({
    ...
    khartoumOnly: tr.querySelector('[data-pmfield="khartoumOnly"]') ? tr.querySelector('[data-pmfield="khartoumOnly"]').checked : false
  }));
